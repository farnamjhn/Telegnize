...
class TestChatPersistence(RepositoryTestCase):
    def test_save_assigns_an_id_and_upserts_by_telegram_id(self):
        chat = self.chats.save(Chat(id=0, telegram_chat_id=123, name="Armita"))
        self.assertTrue(chat.is_persisted)

        again = self.chats.save(Chat(id=0, telegram_chat_id=123, name="Armita Renamed"))
        self.assertEqual(again.id, chat.id)
        self.assertEqual(self.chats.get_by_id(chat.id).name, "Armita Renamed")
